...
export class SearchUserModalState extends State<SearchUserModal> {
    ...
    didMounted(context: BuildContext) {
        ...
        document?.addEventListener('click', async (e) => {
            e.preventDefault();
            if (e.target instanceof HTMLElement && e.target.matches('#search-users-btn')) {
            ...
            if (e.target instanceof HTMLElement && e.target.matches('#next-search-page')) {
                const query = this.searchController.text
            ...
            if (e.target instanceof HTMLElement && e.target.matches('#prev-search-page')) {
                const query = this.searchController.text
                // const query = searchBloc.state.query.trim();
            ...
            if (e.target instanceof HTMLElement && e.target.matches('#close-search-modal')) {
  ...
  `, this.widget.parentId),
            ...
            new BlocListener<SearchBloc, SearchState>({
                blocType: SearchBloc,
