...
export class TournamentRepo extends BaseRepo {
  ...
  getAll(limit: number, offset: number, db?: Database): Tournament[] {
    const database = db ?? this.db;
    const stmt = database.prepare(`
      SELECT id, name, created_by, max_players_count, current_players_count, status, winner
      FROM tournament
  ...
  getAllByStatuses(statuses: string[], limit: number, offset: number, db?: Database): Tournament[] {
    const database = db ?? this.db;
    ...
    const stmt = database.prepare(`
      ...
      LIMIT ? OFFSET ?
    `);
    return stmt.all(...statuses, limit, offset) as Tournament[];
  }

  countAllByStatuses(statuses: string[], db?: Database): number {
    const database = db ?? this.db;
    const placeholders = statuses.map(() => "?").join(",");
    const stmt = database.prepare(`
      SELECT COUNT(*) as count
      ...
      WHERE status IN (${placeholders})
    `);
    const row = stmt.get(...statuses) as { count: number };
    return row.count;
  }
}